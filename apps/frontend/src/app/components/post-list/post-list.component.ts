  ...
  template: `
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div *ngIf="!isLoading && !error" class="space-y-8">
        ...
        <article *ngIf="posts.length > 0" class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
          <div class="md:flex">
            <div class="md:w-1/2 bg-gradient-to-br from-blue-500 to-purple-600 p-8 text-white">
              <div class="h-full flex flex-col justify-center">
                <span class="inline-block bg-white bg-opacity-20 text-white text-xs font-semibold px-3 py-1 rounded-full mb-4">
                  FEATURED
                </span>
                <h2 class="text-2xl md:text-3xl font-bold mb-4 leading-tight">
                  <a [routerLink]="['/post', posts[0].slug]" class="hover:text-blue-100 transition-colors">
                    {{ posts[0].title }}
                  </a>
                </h2>
                <p class="text-blue-100 mb-6 text-lg" *ngIf="posts[0].excerpt">
                  {{ posts[0].excerpt }}
                </p>
            ...
            <div class="md:w-1/2 p-8 bg-gray-50">
              <div class="h-full flex items-center">
                <div class="w-full">
                  <div class="prose prose-gray max-w-none" [innerHTML]="getExcerptHtml(posts[0])">
                  </div>
                  <a [routerLink]="['/post', posts[0].slug]"
        ...
        <!-- Regular Posts Grid -->
        <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3" *ngIf="posts.length > 1">
          <article *ngFor="let post of posts.slice(1); trackBy: trackByPostId"
        ...
        <!-- Pagination -->
        <div *ngIf="paginationData && paginationData.totalPages > 1" class="flex justify-center mt-12">
          <nav class="flex items-center space-x-1">
            <button
              *ngIf="paginationData.hasPrevPage"
            ...
            <span class="px-4 py-2 text-sm font-medium text-gray-700 bg-blue-50 border border-blue-200">
              Page {{ paginationData.page }} of {{ paginationData.totalPages }}
            </span>

            <button
              *ngIf="paginationData.hasNextPage"
